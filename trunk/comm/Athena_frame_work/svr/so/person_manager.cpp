...
namespace PERSON{
	...
	int CPersonManager::AddPerson(Person &person)
	{
		if(!isConneted)
		{
	...
	int CPersonManager::DelPersonById(Person &person)
	{
		if(!isConneted)
		{
			snprintf(m_errmsg, sizeof(m_errmsg), "not conneted to mysql server");
	...
	int CPersonManager::ModifyPerson(Person &person)
	{
		if(!isConneted)
		{
			snprintf(m_errmsg, sizeof(m_errmsg), "not conneted to mysql server");
	...
	int CPersonManager::LiPerson(vector<Person> &person_list)
	{
		if(!isConneted)
		{
			snprintf(m_errmsg, sizeof(m_errmsg), "not conneted to mysql server");
	...
	int CPersonManager::GetPersonInfo(Person &person, int personid)
	{
		if(!isConneted)
		{
			snprintf(m_errmsg, sizeof(m_errmsg), "not conneted to mysql server");
	...
	{
		CPersonManager manager;
		int ret = manager.LiPerson(person_list);
		if(ret !=0)
		{
			errmsg = manager.GetErrMsg();
			return ret;
