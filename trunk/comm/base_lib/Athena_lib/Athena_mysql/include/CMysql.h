...
protected:
    ...
    string _charSet;
    int _connect_timeout_s;//单位是s
    int _read_timeout_s;//单位是s
    int _write_timeout_s;//单位是s
    int _errno;
};
...
struct MySqlData {
    ...
    /**
     *@return insert query后是否返回自动递增id(autoincrement value)，是true否false
     */
...
public: //don't modify these
    ...
    //add by yutaoli 2014年9月25日 23:11:14 begin
    int _autoincrement_id;
    //add by yutaoli 2014年9月25日 23:11:26 end
protected:
